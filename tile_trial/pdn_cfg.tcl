...
# ---------------------------------------------------------------------------
# No macro grid. With a single-layer PDN there is no horizontal layer to hook
# macro power pins to, so instead the macros are placed with their vertical
# power stripes on the tile's stripe grid and the core-grid stripes run
# straight through them (the stock LibreLane script defines a macro grid,
# which makes pdngen carve the core stripes around every macro and leaves the
# macro pins floating). The logical hookup still comes from
# PDN_MACRO_CONNECTIONS via set_global_connections above.
# ---------------------------------------------------------------------------
